...
export default function ChatUI({ isOpen, onClose }) {
  ...
  const handleSend = (text) => {
    ...
    setTimeout(() => {
      ...
      if (lowerText.includes('gadgets') || lowerText.includes('tech') || lowerText.includes('headphones')) {
        botResponse = AI_RESPONSES['recommend gadgets'];
        recommendedProducts = PRODUCTS.filter(p => p.category === 'Tech' || p.category === 'Accessories').slice(0, 2);
      } else if (lowerText.includes('fashion') || lowerText.includes('clothing') || lowerText.includes('apparel')) {
        botResponse = AI_RESPONSES['show fashion items'];
        recommendedProducts = PRODUCTS.filter(p => p.category === 'Clothing' || p.category === 'Accessories').slice(0, 2);
      } else if (lowerText.includes('furniture') || lowerText.includes('home') || lowerText.includes('chair')) {
        botResponse = AI_RESPONSES['furniture'];
        recommendedProducts = PRODUCTS.filter(p => p.category === 'Furniture' || p.category === 'Lighting').slice(0, 2);
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[var(--bg-primary)] transition-all duration-300">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto p-4 md:p-8 space-y-6 container max-w-4xl mx-auto">
              ...
              {msg.products && msg.products.length > 0 && (
                <div className={`flex flex-wrap gap-4 mt-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.products.map(product => (
                    <div key={product.id} className="w-48 sm:w-56 border-2 border-[var(--border-color)] bg-[var(--bg-primary)] group hover:border-[var(--text-primary)] transition-colors text-left">
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-sm font-mono text-[var(--text-secondary)] mb-4">${product.price}</p>
